...
export function getHistory(teamID, week) {
  if (!isProduction) console.log(`/drf/entry/${teamID}/history`); // eslint-disable-line no-console
...
export function getDraftLeague(leagueID = '') {
  if (!isProduction) console.log(`/api/league/${leagueID}/details`); // eslint-disable-line no-console

  return fetch(`https://draft.premierleague.com/api/league/${leagueID}/details`)
    .then(res => res.json());
}

export function getDraftPicks(teamID, week) {
  if (!isProduction) console.log(`/api/entry/${teamID}/event/${week}`); // eslint-disable-line no-console
  if (week === 0) {
    return Promise.resolve({
      entry_history: {
        total_points: 0,
  ...
  return fetch(`https://draft.premierleague.com/api/entry/${teamID}/event/${week}`)
    .then(res => res.json());
}
