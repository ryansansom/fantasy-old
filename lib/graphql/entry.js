    ...
    if (gameweekEnded) {
      if (requestedWeek === currentWeek) {
        return Promise.all([
          ...
          entryPicks,
          currentPoints,
        ]) => entryPicks.entry_history.total_points - currentPoints);
      }

      return this.previousEntryPicks(this.args.week)
        .then(entryPicks => entryPicks.entry_history.total_points);
    }

    return this.previousEntryPicks()
      .then(entryPicks => entryPicks.entry_history.total_points);
  }
