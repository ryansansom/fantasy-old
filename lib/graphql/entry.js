...
export default class Entry extends RootValue {
  constructor(rootValue, playerInfo) {
    ...
    if (playerInfo.previousTotal !== undefined) {
      this._previousTotal = Promise.resolve(playerInfo.previousTotal);
    }
    ...
    if (!this._entryPicksMap.has(week)) {
      this._entryPicksMap.set(
        ...
        this.resources.events.getWeek(week)
          .then(weekNo => this.getPicksLoader.getEntryPicks(this.id, weekNo)),
      );
    }
  ...
  _previousEntryPicks(week) {
    if (!this._previousEntryPicksMap.has(week)) {
      this._previousEntryPicksMap.set(
        week,
        this.resources.events.getWeek(week)
          .then(weekNo => this.getPicksLoader.getEntryPicks(this.id, weekNo - 1)),
      );
    }
  ...
  transferCost() {
    if (!this._transferCost) {
      this._transferCost = this.entryPicks(this.args.week)
        .then(entryPicks => entryPicks.entry_history.event_transfers_cost || 0);
    }
    ...
    if (gameweekEnded) {
      if (requestedWeek === currentWeek) {
        return Promise.all([
          ...
          entryPicks,
          currentPoints,
        ]) => ((entryPicks.entry_history.total_points || 0) - currentPoints));
      }

      return this._previousEntryPicks(this.args.week)
        .then(entryPicks => (entryPicks.entry_history.total_points || 0));
    }

    return this._previousEntryPicks()
      .then(entryPicks => (entryPicks.entry_history.total_points || 0));
  }
