...
export default async(teamID = [], week = '', gwFinished = false) => {
  ...
  return entries.map(entryPicks => {
    const { picks, entry_history: { entry, event_transfers_cost, points, total_points }, active_chip } = entryPicks;
    ...
    picks.forEach((pick, i, picks) => {
      ...
      // TODO: The following line is expecting one fixture match per gameweek, update to take both into account
      const fixture = fixtures.find(game => (elementDetail.team == game.team_a || elementDetail.team == game.team_h)) || noFixtureMatch();
      ...
      pick.actual_bonus = getPlayerBP(fixture.actual_bonus, pick.element);
      pick.provisional_bonus = getPlayerBP(fixture.provisional_bonus, pick.element);
